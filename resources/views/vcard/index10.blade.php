...
@section('base-color')
  <link rel="stylesheet"
    href="{{ asset('assets/front/user/css/vcard/template5-10/vcard-base-color.php') . '?color=' . $vcard->base_color }}">
@endsection
      ...
      <div class="page-area page-area-six">
            ...
            <div class="user-info">
              ...
              @if (!empty($vcard->phone) && (is_array($prefs) && in_array('Add to Contact', $prefs)))
                <a href="{{ route('front.user.vcardImport', [getParam(), $vcard->id]) }}" class="user-btn"><i
                    class="fas fa-plus"></i>{{ $keywords['Add_to_Contact'] ?? 'Add to Contact' }}</a>
              @endif
              ...
              <div class="user-social">
                <ul>
                  ...
                  @if (is_array($prefs) && in_array('Share vCard', $prefs))
                    <li><a href="#" data-toggle="modal" data-target="#socialMediaModal" class="share-btn"><i
            ...
            @foreach ($infos as $info)
              <div class="info-box d-flex">
                ...
                <div class="text">
                  <h3>{{ $info['label'] }}</h3>
                  <p><a
                      @if ($info['link'] == 1) target="_blank" href="{{ $info['value'] }}" @endif>{{ $info['value'] }}</a>
                  </p>
                </div>
              </div>
            @endforeach
        ...
        @if (is_array($prefs) && in_array('Services', $prefs))
          ...
          <div class="section-widget service-area-six mb-25">
            ...
            <div class="service-slider-three">
              @php
                $services = !empty($vcard->user_vcard_services)
                    ? $vcard->user_vcard_services()->orderBy('serial_number', 'ASC')->get()
                    : [];
              @endphp
              @foreach ($services as $service)
                @if ($service->external_link_status == 1 && !empty($service->external_link))
                  ...
                  @elseif ($service->external_link_status == 0 && !empty($service->short_details))
                    <a href="#" data-toggle="modal" data-target="#serviceDetails"
                      class="d-block no-ext service-item" data-details="{{ $service->short_details }}">
                @endif
                <div class="img-holder">
                  <img data-src="{{ asset('assets/front/img/user/services/' . $service->image) }}" alt="Image"
                    class="lazy w-100">
        ...
        @if (is_array($prefs) && in_array('Projects', $prefs))
          ...
          <div class="section-widget project-area-six mb-25">
            ...
            @php
              $projects = !empty($vcard->user_vcard_projects)
                  ? $vcard->user_vcard_projects()->orderBy('serial_number', 'ASC')->get()
                  : [];
            @endphp
            @if (!empty($projects))
              <div class="project-slider-four">
                @foreach ($projects as $project)
                  <div class="project-item">
                    <div class="img-holder">
                      <img data-src="{{ asset('assets/front/img/user/projects/' . $project->image) }}" alt="portfolio"
                        class="w-100 lazy">
                      <a class="img-overlay img-popup"
                        href="{{ asset('assets/front/img/user/projects/' . $project->image) }}">
                        <div class="hover-content">
                    ...
                    @if ($project->external_link_status == 1 && !empty($project->external_link))
                      <a href="{{ $project->external_link }}" target="_blank"
                        class="btn_link d-block text-center">{{ $keywords['Details'] ?? 'Details' }}</a>
                    @elseif ($project->external_link_status == 0 && !empty($project->short_details))
                      <a href="#" data-toggle="modal" data-target="#projectDetails"
                        class="btn_link d-block text-center"
                        data-details="{{ $project->short_details }}">{{ $keywords['Details'] ?? 'Details' }}</a>
                    @endif
                  </div>
                @endforeach
              </div>
          ...
          <div class="section-widget intro-area-six mb-25">
            @if (!empty($vcard->video))
              <div class="intro-item">
                <div class="img-holder">
                  <iframe class="embed-responsive-item" src="{{ $vcard->video }}" allowfullscreen width="100%"
                    height="250"></iframe>
                </div>
              </div>
            @endif
        ...
        @if (is_array($prefs) && in_array('Testimonials', $prefs))
          ...
          <div class="section-widget testimonial-area-six mb-25">
            @php
              $testimonials = !empty($vcard->user_vcard_testimonials)
                  ? $vcard->user_vcard_testimonials()->orderBy('serial_number', 'ASC')->get()
                  : [];
            @endphp
              ...
              <form action="{{ route('front.contact.message', getParam()) }}" method="POST"
                ...
                <div class="row">
                  <div class="col-12">
                    <div class="form_group">
                      <input type="text" class="form_control"
                        placeholder="{{ $keywords['Enter_Name'] ?? 'Enter_Name' }}" name="fullname" required>
                  ...
                  <div class="col-12">
                    <div class="form_group">
                      <input type="email" class="form_control"
                        placeholder="{{ $keywords['Enter_Email_Address'] ?? 'Enter_Email_Address' }}" name="email"
                        required>
                      @if ($errors->has('email'))
                  ...
                  <div class="col-12">
                    <div class="form_group">
                      <input type="text" class="form_control"
                        placeholder="{{ $keywords['Enter_Subject'] ?? 'Enter_Subject' }}" name="subject" required>
